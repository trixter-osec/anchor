...
pub fn generate(
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                if f.is_optional {
                    quote! {
                        #docs
                        pub #name: Option<trixter_osec_anchor_lang::solana_program::account_info::AccountInfo<'info>>
                    }
                } else {
                    quote! {
                        #docs
                        pub #name: trixter_osec_anchor_lang::solana_program::account_info::AccountInfo<'info>
                    }
                }
            }
        })
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                let meta = match f.constraints.is_mutable() {
                    false => quote! { trixter_osec_anchor_lang::solana_program::instruction::AccountMeta::new_readonly },
                    true => quote! { trixter_osec_anchor_lang::solana_program::instruction::AccountMeta::new },
                };
                let name = &f.ident;
                if f.is_optional {
                    quote! {
                        if let Some(#name) = &self.#name {
                            account_metas.push(#meta(trixter_osec_anchor_lang::Key::key(#name), #is_signer));
                        } else {
                            account_metas.push(trixter_osec_anchor_lang::solana_program::instruction::AccountMeta::new_readonly(#program_id, false));
                        }
                    }
                } else {
                    quote! {
                        account_metas.push(#meta(trixter_osec_anchor_lang::Key::key(&self.#name), #is_signer));
                    }
                }
            }
        })
    ...
    let account_struct_infos: Vec<proc_macro2::TokenStream> = accs
        ...
        .map(|f: &AccountField| {
            let name = &f.ident();
            quote! {
                account_infos.extend(trixter_osec_anchor_lang::ToAccountInfos::to_account_infos(&self.#name));
            }
        })
        .collect();
    ...
    quote! {
        ...
        pub(crate) mod #account_mod_name {
            ...
            impl #generics trixter_osec_anchor_lang::ToAccountMetas for #name #generics {
            ...
            #[automatically_derived]
            impl<'info> trixter_osec_anchor_lang::ToAccountInfos<'info> for #name #generics {
                fn to_account_infos(&self) -> Vec<trixter_osec_anchor_lang::solana_program::account_info::AccountInfo<'info>> {
