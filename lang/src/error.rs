...
impl ProgramErrorWithOrigin {
    pub fn log(&self) {
        match &self.error_origin {
            None => {
                trixter_osec_anchor_lang::solana_program::msg!(
            ...
            Some(ErrorOrigin::AccountName(account_name)) => {
                ...
                trixter_osec_anchor_lang::solana_program::log::sol_log(&format!(
        ...
        match &self.compared_values {
            Some(ComparedValues::Pubkeys((left, right))) => {
                trixter_osec_anchor_lang::solana_program::msg!("Left:");
                left.log();
                trixter_osec_anchor_lang::solana_program::msg!("Right:");
                right.log();
            }
            Some(ComparedValues::Values((left, right))) => {
                trixter_osec_anchor_lang::solana_program::msg!("Left: {}", left);
                trixter_osec_anchor_lang::solana_program::msg!("Right: {}", right);
            }
            None => (),
        }
    }
...
impl std::convert::From<Error> for trixter_osec_anchor_lang::solana_program::program_error::ProgramError {
    fn from(e: Error) -> trixter_osec_anchor_lang::solana_program::program_error::ProgramError {
        match e {
            Error::AnchorError(error) => {
                trixter_osec_anchor_lang::solana_program::program_error::ProgramError::Custom(
