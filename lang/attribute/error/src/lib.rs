...
/// Generates an [`Error::AnchorError`](../../trixter_osec_anchor_lang/error/enum.Error.html) that includes file and line information.
///
/// # Example
/// ```rust,ignore
/// #[program]
...
fn create_error(error_code: Expr, source: bool, account_name: Option<Expr>) -> TokenStream {
    let error_origin = match (source, account_name) {
        ...
        (false, Some(account_name)) => quote! {
            Some(trixter_osec_anchor_lang::error::ErrorOrigin::AccountName(#account_name.to_string()))
        },
        (true, _) => quote! {
            Some(trixter_osec_anchor_lang::error::ErrorOrigin::Source(trixter_osec_anchor_lang::error::Source {
                filename: file!(),
                line: line!()
            }))
        },
