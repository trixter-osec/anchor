...
pub fn initialize_account<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(
        &ix,
        &[
...
pub fn initialize_account3<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account, ctx.accounts.mint])
        .map_err(Into::into)
}
...
pub fn initialize_mint<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint, ctx.accounts.rent])
        .map_err(Into::into)
}
...
pub fn initialize_mint2<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint]).map_err(Into::into)
}
...
pub fn sync_native<'info>(ctx: CpiContext<'_, '_, '_, 'info, SyncNative<'info>>) -> Result<()> {
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account]).map_err(Into::into)
}
...
pub fn get_account_data_size<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint])?;
    trixter_osec_anchor_lang::solana_program::program::get_return_data()
        .ok_or(trixter_osec_anchor_lang::solana_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != ctx.program_id {
                Err(trixter_osec_anchor_lang::solana_program::program_error::ProgramError::IncorrectProgramId)
            } else {
                data.try_into().map(u64::from_le_bytes).map_err(|_| {
                    trixter_osec_anchor_lang::solana_program::program_error::ProgramError::InvalidInstructionData
                })
            }
        })
        .map_err(Into::into)
...
pub fn amount_to_ui_amount<'info>(
    ...
    trixter_osec_anchor_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account])?;
    trixter_osec_anchor_lang::solana_program::program::get_return_data()
        .ok_or(trixter_osec_anchor_lang::solana_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != ctx.program_id {
...
impl trixter_osec_anchor_lang::Id for Token2022 {
    fn id() -> Pubkey {
        ID
    }
}
