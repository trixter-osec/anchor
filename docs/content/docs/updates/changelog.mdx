...
- client: Add option to pass in mock rpc client when using trixter_osec_anchor_client
...
- lang: Add `trixter_osec_anchor_lang::pubkey` macro for declaring `Pubkey` const values
...
- ts: Add `.interface(..)` method for instruction discriminator overrides
  ([#2728](https://github.com/coral-xyz/anchor/pull/2728)).
- cli: Check `trixter-osec-anchor-lang` and CLI version compatibility
...
- lang: `Box` the inner enums of `trixter_osec_anchor_lang::error::Error` to optimize
...
- lang: Add `PartialEq` and `Eq` for `trixter_osec_anchor_lang::Error`
...
- lang: Adjust error code so `#[error_code]` works with just importing
  `trixter_osec_anchor_lang::error_code`
...
- lang: add
  [`error!`](https://docs.rs/trixter-osec-anchor-lang/latest/trixter_osec_anchor_lang/prelude/macro.error.html)
  and
  [`err!`](https://docs.rs/trixter-osec-anchor-lang/latest/trixter_osec_anchor_lang/prelude/macro.err.html)
